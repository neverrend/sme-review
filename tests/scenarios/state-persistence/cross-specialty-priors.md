...
2. Run the first invocation (security/network review). Let it complete. Verify a state file was written at `<cwd>/.claude/sme-reviews/order-ingestion-service/security/network.md`.
...
**Expected behavior:**
- On the second invocation, the continuation reader lists the state files in `<cwd>/.claude/sme-reviews/order-ingestion-service/` recursively and finds `security/network.md` from the first run.
