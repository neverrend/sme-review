...
3. **State persistence** (`scenarios/state-persistence/`): verify cache-hit/miss, cross-specialty priors, force re-review, concurrent lockfile rejection.
4. **Extraction** (`scenarios/extraction/`): verify unambiguous / ambiguous-confirmation / no-section-refusal flows.
5. **Edge cases** (`scenarios/edge-cases/`): verify multi-specialty confirmation, read-only cwd fallback, state write failure fallback, retry policy.
